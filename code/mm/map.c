...
{
  //trace_printf(" adding memory entry: %0#18x - %0#18x [%s]\n", addr_start, addr_end, mm_map_type_desc(type));
...
list_t *mm_map_init(multiboot_t *multiboot)
{
  ...
  while (entry_addr < entry_limit)
  {
    ...
    if (entry->length > 0)
      mm_map_add(entry->type, entry->base_addr, entry->base_addr + entry->length - 1);
  ...
  /* print the final map */
  trace_printf("Final physical memory map:\n");
  list_for_each(&entry_list, node)
  {
    mm_map_entry_t *entry = container_of(node, mm_map_entry_t, node);
